...
class formlist {
    ...
    public function addVSpace($size = 50) {
        $this->content.= "<ul><li><div style=\"height:" . $size . "px;\"></div></li></ul>\n";
    }
    ...
    public function addDiv($label, $content, $props = array(), $newrow = false) {
        if ($newrow) {
            $this->content.= '<ul>';
        } else {   //rimuovo il prec </ul> se è presente 
            $tr_pos = strlen($this->content) - strrpos($this->content, "</ul>");
            // echo strrpos($this->content,"</ul>")."   -   ".(strlen($this->content)-6); 
            //da implementare con le regexp
            if ($tr_pos < 8) {   //solo se l' ultimo <ul/> si trova meno di 8 caratteri prima della fine della stringa.
                //elimino "</ul>\n" presente alla fine della stringa
                $this->content = substr($this->content, 0, strlen($this->content) - $tr_pos);
            }
        }//close else  

        $code = "<li><label>" . $label . "</label><br /><div";
        if (is_array($props)) {
            foreach ($props as $key => $value)
                $code.=" $key = \"$value\"";
        }
        $this->content.= $code . ">" . $content . "</div></li></ul>\n";
    }
...
//close function
    ...
    public function addTextField($name, $label, $properties = array()) {
        ...
        $this->content.= "\n";
        $this->content.= "<ul><li>$label</li><li><input type=\"text\" tabindex=\"" . $this->tabindex++ . "\" name=\"$name\" $props /></li></ul>\n";
    }
    ...
    public function addTextField_2($name, $label, $properties = array(), $newrow = true, $colspan = array("", "", "")) {
       ...
       if ($colspan[0] != "") {
        ...
        $this->content.= "\n";
        $this->content.= "<li" . $colspan[1] . "><label>$label</label><br /><input " . $type . " tabindex=\"" . $this->tabindex++ . "\" name=\"$name\" $props /></li></ul>\n";
    }
    ...
    public function addTextArea($name, $label, $properties = array(), $newrow = true, $colspan = array("", "", "")) {
        ...
        $this->content.= "\n";
        $this->content.= "<li" . $colspan[1] . "><label>$label</label><br /><textarea tabindex=\"" . $this->tabindex++ . "\" name=\"$name\" $props>" . $sinserttext . "</textarea></li></ul>\n";
    }
    ...
    public function addPassField($name, $label, $minlength, $repeat, $properties = array()) {
        ...
        $this->content.= "\n";
        $this->content.= "<ul><li><label>$label</label><br /><input type=\"password\"  tabindex=\"" . $this->tabindex++ . "\" name=\"$name\"  id=\"$id_passfield\" $props  /></li></ul>\n";
        if ($repeat == 1) {
            $props = str_replace("/" . $id_passfield . "/", "confirm_$name", $props);
            $this->content.= "<ul><li><label>Conferma $label</label><br /><input type=\"password\" tabindex=\"" . $this->tabindex++ . "\" name=\"confirm_$name\" id=\"$id_confirm_passfield\" $props/></li></ul>\n";
    ...
    public function addSelectBox_2($name, $label, $optionslist, $properties = array(), $selectedvalue = "", $newrow = true, $colspan = array("", "", "")) {
        ...
        if ($label == "")
            $html = "<li" . $colspan[2] . "><select name=\"$name\">\n";
        else
            $html = "<li" . $colspan[1] . ">$label</li><li" . $colspan[2] . "><select name=\"$name\" tabindex=\"" . $this->tabindex++ . "\">";
        ...
        foreach ($optionslist as $key => $value) {
            if ($value != $selectedvalue) {
                ...
                $this->content.= '<option value="' . $value . '" selected="selected">' . $value . '</option>' . "\n";
            }
        }
        $this->content.= "</select></li></ul>\n";
    }
    ...
    public function addMultiSelectBox2($name, $label, $optionslist, $properties = array(), $selectedvalue = "") {
        ...
        if ($label == "") {
            ...
            $html = "<ul><li>$label</li><li><select id=\"$name\" name=\"$name\" tabindex=\"" . $this->tabindex++ . "\" multiple=\"multiple\">\n";
        }
        if ((!is_array($properties)) && (!is_null($properties))) {
            $selectedvalue = $properties;
        } else if (count($properties) > 0) {
    ...
    public function addMultiSelectBoxManager($name, $label, $optionslist, $properties = array(), $selectedvalue = "") {
     ...
     ");
        ...
        if ($label == "") {
            ...
            $html = "<ul><li>$label</li><li><select name=\"$name\" id=\"$name\" tabindex=\"" . $this->tabindex++ . "\" multiple=\"multiple\">";
        }
        if ((!is_array($properties)) && (!is_null($properties))) {
            $selectedvalue = $properties;
        } else if (count($properties) > 0) {
        ...
        $this->content.= '</select><br /><a id="plus" href="javascript:;" tabindex="' . $this->tabindex++ . '"><img src="/' . LIBSSDIR . 'icons/plus.png" alt="+"></a>
                     <a id="minus" href="javascript:;" tabindex="' . $this->tabindex++ . '"><img src="/' . LIBSSDIR . 'icons/minus.png" alt="-"></a></li></ul>';
        ...
        //alert(\'Errore: \' + errorThrown ? errorThrown : xhr.status);});" tabindex="'.$this->tabindex++.'"><img src="/'.LIBSSDIR.'icons/plus.png" alt="+"></a><a href="#" tabindex="'.$this->tabindex++.'"><img src="/'.LIBSSDIR.'icons/minus.png" alt="-"></a></li></ul>';
    }
    ...
    public function addNumberSelector($name, $label, $url, $title, $value = "") {
      ...
      function clearNumbersField(name){
        ...
        $this->content.="<ul><li>$label";
        $this->content.="<input type=\"hidden\" name=\"$name\" value=\"$value\">";
        $linktext = ($value == "") ? "Seleziona" : $value;
        $this->content.="</li><li id=\"$name\"><a tabindex=\"" . $this->tabindex++ . "\" href=\"javascript:openNumberSelector('$url','$title',700,600);void(0);\">$linktext</a>\n";
        $this->content.="</li><li><a href=\"javascript:clearNumbersField('$name')\">Cancella</a></li></ul>";
    }
    ...
    public function addCaptcha($label, $properties = array()) {
        ...
        $this->content.="<ul>\n<li><label>$label</label><br /><input type=\"text\" tabindex=\"" . $this->tabindex++ . "\" name=\"captcha\" " . $props . " /><br /><br />\n<img src=\"../Lib/captcha.php\" /><li></ul>";
    ...
    public function addMultiTextField($name, $label, $properties = array()) {
        ...
        $this->content.= "\n";
        $name.="[]";
        $this->content.= "<ul><li>$label</li><li><input type=\"text\" tabindex=\"" . $this->tabindex++ . "\" name=\"$name\" $props /></li><li><a href=\"javascript:addElement('$name');\">Aggiungi</a></li></ul>\n";
    }
    ...
    public function addMultiTextWithSelect($name, $label, $properties = array()) {
        ...
        $this->content.= "\n";
        $name.="[]";
        $this->content.= "<ul><li>$label</li><li><input type=\"text\" tabindex=\"" . $this->tabindex++ . "\" name=\"$name\" $props /><select name=\"select_$name\"><option name=\"null\" value=\"null\">Seleziona piano</option>';  </select><li><a href=\"javascript:addElement('$name');\">Aggiungi</a></li></ul>\n";
    }
    ...
    public function addClientSelector($field = array(), $idContainerName, $properties = array()) {
      ...
      ';
        ...
        $this->content.='<ul><li>' . $field[1] . '</li><li><input type="text" tabindex="' . $this->tabindex++ . '" id="' . $field[0] . '" name="' . $field[0] . '" autocomplete="off" ' . $props . '>
    ...
    public function addFileSelector($name, $label, $url, $title, $value = "") {
        ...
        $this->content.="</li><li id=\"$name\"><a tabindex=\"" . $this->tabindex++ . "\"  href=\"javascript:openFileSelect('$url','$title',700,600);void(0)\">$linktext</a>\n";
        $this->content.="</li><li><a href=\"javascript:clearFileName('$name')\">Elimina</a></li></ul>";
    }
    ...
    public function addDoubleSelectBox($names, $labels, $optionslist1, $optionslist2, $properties = array(), $selectedvalues = "") {
        ...
        //$html="<ul><li>$label</li><li><select name=$name>";
        $html = "
		<ul>
      	  <li>$labels[0]</li>
		  <li>&nbsp;</li>
    ...
    public function addHideField($name, $value, $properties = array(), $newrow=true) {
        ...
        $this->content.="<li><input type=\"hidden\" name=\"$name\" value=\"$value\" $props /></li></ul>\n";
    }
    ...
    public function addRule($fieldname, $requisite, $errormsg = "") {
        ...
        if ($this->rulesActive == false) {
            ...
            $this->addJS(LIBSSDIR . "js/validate.js");  //file con funzioni di validazione
            ...
            $this->content.='<ul><li><div id="errorDiv" align="center"></div></li></ul>';
//Il parent è l' oggetto page
    ...
    public function addButton_2($type = "submit", $buttonname = "Invia", $properties = array(), $columnprefix = 1, $newrow = true, $colspan = "") {
        ...
        if (!$newrow && $tr_pos < 8) {   //solo se l' ultimo <ul/> si trova meno di 8 caratteri prima della fine della stringa.
            //elimino "</ul>\n" presente alla fine della stringa
            $this->content = substr($this->content, 0, strlen($this->content) - $tr_pos);
        }
    ...
    public function addRadioButton($label_title, $labels, $content, $vertical = true) {
        if ($vertical) {
            $this->content.= "\n\n<ul><li><fieldset><legend>" . $label_title . "</legend><br /><center><table>\n";
            for ($i = 0; $i < count($content); $i++) {
                $this->content.= "<ul><li><input type=\"radio\"";
                foreach ($content[$i] as $k => $v) {
                    $this->content.= " " . $k . "=\"" . $v . "\"";
                }
                $this->content.= ">" . $labels[$i] . "</input></li></ul>\n";
            }//close for
            $this->content.= "</center></fieldset></li></ul>\n\n";
        } else {                                //if not vertical
            $this->content.= "\n\n<ul><li colspan=3><fieldset><legend>" . $label_title . "</legend><br /><center><table><ul>\n";
            for ($i = 0; $i < count($content); $i++) {
                $this->content.= "<li><input type=\"radio\"";
                foreach ($content[$i] as $k => $v) {
                    $this->content.= " " . $k . "=\"" . $v . "\"";
                }
                $this->content.= " />" . $labels[$i] . "</li>\n";
            }//close for
            $this->content.= "</ul></center></fieldset></li></ul>\n\n";
        }//close else
    }
    ...
    public function addCheckBox($name, $label, $properties = array(), $newrow = true, $colspan = array("", "", "")) {
        ...
        $this->content.= "\n";
        // $this->content.= "<li".$colspan[1]."><input ".$type." tabindex=\"".$this->tabindex++."\" name=\"$name\" onFocus=\"this.style.backgroundColor='".$this->focuscolor."'\" onBlur=\"this.style.backgroundColor=''\" $props /></li><li".$colspan[2].">".$label."</li></ul>\n";
        $this->content.= "<li" . $colspan[1] . "><input " . $type . " tabindex=\"" . $this->tabindex++ . "\" name=\"$name\" $props /></li><li" . $colspan[2] . ">" . $label . "</li></ul>\n";
    }
    ...
    public function addRisultSearch1($value = "", $newrow = true) {
        if ($newrow) {
